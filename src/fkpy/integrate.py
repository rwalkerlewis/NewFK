...
table to :func:`wavenumber_sum_loop_tabulated`.
"""
...
@nb.njit(cache=True)
def wavenumber_sum_loop_tabulated(  # noqa: PLR0913, PLR0915
    k_array: F64Array,
    ...
    *outside* this routine, where ``Δk`` is known.
    ...
    The Bessel table ``bessel_table[ik, idist, m]`` for ``m = 0, 1, 2``
    is precomputed on the host (see :func:`fkpy.bessel.precompute_bessel`)
    so this routine is fully Numba-jittable.
    """
    n_k = k_array.size
